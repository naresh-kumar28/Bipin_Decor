...
function AdminSettings() {
  ...
  const [newSocial, setNewSocial] = useState({ platform_name: '', url: '', custom_icon_svg: '' });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Object.keys(settings).forEach(key => {
        ...
        // Skip read-only image/file URL fields generated by serializer
        const isFileUrlField = key.endsWith('_url') && fileFields.some(f => key.startsWith(f));
        if (isFileUrlField) return;
  ...
  const handleAddSocial = async () => {
    ...
    try {
      ...
      setNewSocial({ platform_name: '', url: '', custom_icon_svg: '' });
      fetchSocialLinks();
    } catch (err) {
            ...
            {activeTab === 'social' && (
              <>
                ...
                <div className="space-y-6 pt-4 border-t border-border">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4 bg-muted/30 rounded-xl border border-border">
                    <div>
                    ...
                    <div className="flex items-end gap-2">
                      <div className="flex-1">
                        <label className="block text-[10px] font-bold text-muted-foreground uppercase mb-1 ml-1">Custom SVG Code</label>
                        <input 
                          type="text" 
                          placeholder="<svg..."
                          value={newSocial.custom_icon_svg || ''}
                          onChange={(e) => setNewSocial({...newSocial, custom_icon_svg: e.target.value})}
                          className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm focus:ring-1 focus:ring-primary outline-none"
                        />
                      </div>
                      <button 
